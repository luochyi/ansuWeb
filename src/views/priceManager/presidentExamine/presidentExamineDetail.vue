...
export default {
  ...
  methods: {
    ...
    adopt () {
      this.$api.cost.price.examine.managerAdopt({ waybillId: this.waybillId }).then(res => {
        if (res.code === 0) {
    ...
    rejectSubmit () {
      this.$api.cost.price.examine.managerReject({ waybillId: this.waybillId, rejectReason: this.rejectReason }).then(res => {
        if (res.code === 0) {
          this.$message.success(res.msg)
          this.back()
        } else {
